<template>
  <div class="group h-[420px] w-full perspective-1000 cursor-pointer">
    <div class="relative h-full w-full transition-transform duration-700 preserve-3d group-hover:rotate-y-180">
      ...
      <div class="absolute inset-0 backface-hidden flex flex-col items-center justify-center p-8 bg-white rounded-2xl shadow-sm border border-berlin-light">
        ...
        <p class="mt-6 text-berlin-grey text-xs uppercase tracking-widest font-semibold">Survoler pour les détails</p>
      </div>
      ...
      <div class="absolute inset-0 backface-hidden rotate-y-180 flex flex-col p-8 bg-berlin-blue text-white rounded-2xl shadow-xl">
        ...
        <button @click.stop="$emit('reserve', course.id)" class="btn-primary w-full mt-6 shadow-lg shadow-berlin-gold/20">
          Réserver ma place
        </button>
      </div>
